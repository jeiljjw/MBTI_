...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = isEn
    ? "MBTI Conflict Resolution at Work: Type-Based Strategies | MBTI Blog"
    : "MBTI 직장에서의 갈등 해결: 유형별 갈등 관리 전략 | MBTI 블로그";
  const description = isEn
    ? "How each of the 16 MBTI types handles conflict. Manage workplace conflict and communicate better by understanding type differences."
    : "16가지 MBTI 유형별 갈등 해결 방식을 분석합니다. 직장에서의 갈등을 효과적으로 관리하고, 유형 차이를 이해하여 더 나은 커뮤니케이션을 하는 방법을 알아봅니다.";
  return {
    title,
    description,
    keywords: isEn
      ? ["MBTI conflict", "workplace conflict", "conflict by type", "MBTI communication"]
      : ["MBTI 갈등", "직장 갈등 해결", "유형별 갈등 관리", "MBTI 커뮤니케이션", "직장 갈등"],
    openGraph: { title, description, type: "article" },
  };
}

function ConflictResolutionKo({ t }: { t: (key: string) => string }) {
  return (
    <>
...
function ConflictResolutionEn({ t }: { t: (key: string) => string }) {
  return (
    <>
      <nav className="text-sm text-gray-400 mb-6">
        <ol className="flex items-center space-x-2">
          ...
          <li><Link href="/blog" className="hover:text-white transition-colors">{t("blog")}</Link></li>
          <li>&gt;</li>
          <li className="text-white">Conflict Resolution at Work</li>
        </ol>
      </nav>
      <header className="mb-10">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-3 py-1 bg-purple-500/20 text-purple-300 rounded-full text-sm">Career</span>
      ...
      <div className="bg-black/20 backdrop-blur-md rounded-xl p-6 border border-white/10 mb-10">
        <h2 className="text-lg font-bold text-white mb-4">{t("tocTitle")}</h2>
        <ul className="space-y-2 text-gray-400">
          <li><a href="#conflict-basics" className="hover:text-white transition-colors">1. Conflict and MBTI</a></li>
          <li><a href="#nt-conflict" className="hover:text-white transition-colors">2. Analyst (NT) Types</a></li>
          <li><a href="#nf-conflict" className="hover:text-white transition-colors">3. Diplomat (NF) Types</a></li>
          <li><a href="#sj-conflict" className="hover:text-white transition-colors">4. Sentinel (SJ) Types</a></li>
          <li><a href="#sp-conflict" className="hover:text-white transition-colors">5. Explorer (SP) Types</a></li>
          <li><a href="#cross-type" className="hover:text-white transition-colors">6. Cross-Type Conflict Patterns</a></li>
          <li><a href="#resolution-strategies" className="hover:text-white transition-colors">7. Effective Resolution Strategies</a></li>
        </ul>
      </div>
      <div className="prose prose-invert prose-lg max-w-none">
        <section id="conflict-basics" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">1. Conflict and MBTI</h2>
          <p className="text-gray-300 leading-relaxed mb-4">
          ...
          <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4 my-6">
            <p className="text-purple-200 text-sm">
              <strong>Key point:</strong> Conflict isn&apos;t bad. Handled well, it can lead to better solutions, stronger bonds, and deeper understanding.
            </p>
          </div>
        </section>
        <section id="nt-conflict" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">2. Analyst Types (NT)</h2>
        ...
        <section id="nf-conflict" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">3. Diplomat Types (NF)</h2>
          <p className="text-gray-300 leading-relaxed mb-4">Diplomats view conflict in relational terms. They value harmony. INFJ: seeks harmony; can react strongly when pushed. INFP: processes conflict inwardly. ENFJ: often mediates; may neglect own needs. ENFP: may avoid or express intensely. Tips: talk one-on-one; respect values; combine empathy with clear solutions.</p>
        </section>
        <section id="sj-conflict" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">4. Sentinel Types (SJ)</h2>
          <p className="text-gray-300 leading-relaxed mb-4">Sentinels see conflict through rules and systems. They value tradition and order. ISTJ: careful, traditional; may resist change. ISFJ: avoids conflict; can push back when treated unfairly. ESTJ: direct and efficient; may overlook emotions. ESFJ: values harmony and relationships. Tips: respect procedures; give fact-based feedback; show appreciation.</p>
        </section>
        <section id="sp-conflict" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">5. Explorer Types (SP)</h2>
          <p className="text-gray-300 leading-relaxed mb-4">Explorers take a practical, here-and-now view. ISTP: calm and analytical; can seem indifferent. ISFP: quiet and flexible; prefers influence over force. ESTP: energetic; may react impulsively. ESFP: spontaneous and emotional; may avoid or blow up. Tips: respect space; focus on practical solutions; avoid emotional pressure.</p>
        </section>
        <section id="cross-type" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">6. Cross-Type Conflict Patterns</h2>
          <p className="text-gray-300 leading-relaxed mb-4">Certain type pairs clash often. E vs I: &quot;Too quiet&quot; vs &quot;Too loud&quot;—communication style. S vs N: &quot;Be specific&quot; vs &quot;Show the big picture&quot;—information style. T vs F: logic vs emotion in decisions. J vs P: &quot;Stick to the plan&quot; vs &quot;Stay flexible.&quot;</p>
        </section>
        <section id="resolution-strategies" className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6">7. Effective Resolution Strategies</h2>
          <div className="space-y-4">
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">1. Know your type</p><p className="text-gray-300 text-sm">Understanding how you react in conflict helps you manage weaknesses.</p></div>
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">2. Understand the other type</p><p className="text-gray-300 text-sm">The same event looks different to different types. Seek their perspective.</p></div>
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">3. Separate emotion from the issue</p><p className="text-gray-300 text-sm">Breathe and focus on the core problem.</p></div>
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">4. Match communication style</p><p className="text-gray-300 text-sm">Use facts with T types, empathy and values with F types.</p></div>
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">5. Aim for win-win</p><p className="text-gray-300 text-sm">Find solutions that work for both sides.</p></div>
            <div className="bg-purple-900/20 border-l-4 border-purple-500 p-4"><p className="text-purple-200 font-semibold mb-1">6. Get professional help when needed</p><p className="text-gray-300 text-sm">Mediators or coaches can help with complex or long-standing conflict.</p></div>
          </div>
        </section>
      </div>
      <div className="border-t border-gray-800 pt-6 mb-10">
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Link href="/blog/mbti-leadership" className="group">
            <div className="bg-black/20 backdrop-blur-md rounded-lg p-4 border border-white/10 hover:border-purple-400 transition-all">
              <h4 className="font-semibold text-white group-hover:text-purple-400 mb-1">MBTI Leadership Styles</h4>
              <p className="text-gray-400 text-sm">Communication and influence by type</p>
            </div>
          </Link>
          <Link href="/blog/mbti-compatibility-truth" className="group">
            <div className="bg-black/20 backdrop-blur-md rounded-lg p-4 border border-white/10 hover:border-pink-400 transition-all">
              <h4 className="font-semibold text-white group-hover:text-pink-400 mb-1">The Truth About MBTI Compatibility</h4>
              <p className="text-gray-400 text-sm">What the data says</p>
            </div>
          </Link>
        </div>
      </section>
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pt-24 pb-16">
      <article className="max-w-4xl mx-auto px-4">
        {isEn ? <ConflictResolutionEn t={t} /> : <ConflictResolutionKo t={t} />}
      </article>
    </div>
  );
}
